...
namespace Invector.CharacterController
{
    public abstract class vThirdPersonMotor : MonoBehaviour
    {
        ...
        public virtual void UpdateMotor()
        {
            ...
            ControlLocomotion();
            CheckIsAttacking();
            ControlBlowBehaviour();
            ControlFreezingBehaviour();
        ...
        void ControlFreezingBehaviour()
        {
            if(animator.GetCurrentAnimatorStateInfo(0).IsName("Attack0") ||
                animator.GetCurrentAnimatorStateInfo(0).IsName("Attack1") ||
                animator.GetCurrentAnimatorStateInfo(0).IsName("Attack2") ||
                 isKicked || isAttacked || isBlocking)
            { 
                speed = 0f;
            }
        }

        void ControlBlowBehaviour()
        {

            if (isAttacked)
            {
                if (!animator.GetCurrentAnimatorStateInfo(0).IsName("Blow"))
                {
        ...
        void ControlLocomotion()
        {
            if(!(animator.GetCurrentAnimatorStateInfo(0).IsName("Attack0") ||
                animator.GetCurrentAnimatorStateInfo(0).IsName("Attack1") ||
                animator.GetCurrentAnimatorStateInfo(0).IsName("Attack2")) && !isAttacked && !isBlocking && !isAttacking && !isKicked)
            {
